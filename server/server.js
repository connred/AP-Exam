...
// GOOGLE SIGN IN CODE FROM GOOGLES DOCUMENTATION
...
io.sockets.on('connection', function (socket) {
    ...
    socket.on('adduser', function (login) {
        ...
        socket.join('Primary');
        socket.emit('updatechat', 'CONSOLE', {
            message: 'you have connected to Primary'
        });
        socket.broadcast.to('Primary').emit('updatechat', 'CONSOLE', {
            message: login.name + ' has connected to ' + socket.room
        });
    ...
    socket.on('switchRoom', function (newroom) {
        ...
        socket.broadcast.to(socket.room).emit('updatechat', 'CONSOLE', {
            message: socket.user + ' has left this room'
        });
        socket.room = newroom;
        socket.broadcast.to(newroom).emit('updatechat', 'CONSOLE', {
    ...
    socket.on('disconnect', function () {
        ...
        socket.broadcast.emit('updatechat', 'CONSOLE', {
            message: socket.user + ' has disconnected'
        });
        socket.leave(socket.room);
    });
    //
